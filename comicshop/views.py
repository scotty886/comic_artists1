...
def users_list(request):
    #logger.error(f'***** AN ERROR OCCURRED AT USERS LIST: {request.path}')
    users = User.objects.all()
    return render(request, 'users_list.html', {'users': users})  # Pass the users list to the template


def add_product(request):
    logger.error(f'***** AN ERROR OCCURRED AT ADD PRODUCT: {request.path}')
    if request.method == 'POST':
        ...
        form = ProductForm()
    return render(request, 'add_product.html', {'form': form})  # Pass the form to
